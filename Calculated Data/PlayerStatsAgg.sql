select 
    coalesce(pr.player_name,pb.player) player
    ,pb.player_id
    ,date_part('year', age(date(concat('02-01-',cast(gs.season as char(4)))), cast(pr.dob as date))) as age
    ,tsa.team
    ,pb.team_abbr
    ,tsa.team_id
    ,gs.season
...
where 1=1
    ...
    and pb.mp > 0
    and gs.game_type in ('Regular Season','Postseason')
group by
    coalesce(pr.player_name,pb.player)
    ,pb.player_id
    ,date_part('year', age(date(concat('02-01-',cast(gs.season as char(4)))), cast(pr.dob as date)))
    ,tsa.team
    ,pb.team_abbr
    ,tsa.team_id
    ,gs.season
